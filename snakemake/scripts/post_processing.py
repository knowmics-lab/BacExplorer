...
# path del container
# PATH_OUTPUT = "/project/user-input/output"
...
def delete_intermediate_files(directory, tool_name):
    for root, dirs, files in os.walk(directory):
        for file in files:
            if tool_name == "mlst" and "_result" in file:
                file_path = os.path.join(root, file)
                print(f"Deleting temp file: {file_path}")
                os.remove(file_path)
            if tool_name == "kraken2" and "_result" in file:
                file_path = os.path.join(root, file)
                print(f"Deleting temp file: {file_path}")
                os.remove(file_path)
