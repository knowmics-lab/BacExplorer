...
#for Rstudio testing
# path_output <- normalizepath("D:\\Uni\\BacExplorer\\campioni\\fastq_test_2", winslash = "/", mustwork = true)
# path_output <- "D:/Uni/BacExplorer/campioni/fastq_test_2"
...
type <- tolower(trimws(params$type))
...
try({
  ...
  fq_tab <- as.data.frame(do.call(bind_rows, lapply(fastqc, fastqc_function)))
...
quast <- list.files(path=paste0(path_output, "/quality_assessment/quast_results/"), pattern="_report.tsv", full.names=TRUE, recursive=TRUE)
 rs <- grep("reads_stats", quast)
 quast <- quast[-c(rs)]
 aq <- grep("transposed_", quast)
 quast <- quast[-c(aq)]

 quast_function <- function(...) {
  stop("quast_function not initialized. type=", type)
}
 ...
 if(type == "fasta"){
  ...
  "Properly paired (%)", "Avg. coverage depth", "Coverage >= 1x (%)", "N's per 100 kbp", "Sample")
    qs <- qs[,c(17,1:9, 15, 13, 14, 16)]
      return(qs)
      }
   }
  }
  
  quast_tab <- as.data.frame(do.call(rbind, lapply(quast, quast_function)))
  rownames(quast_tab) <- NULL
  
  # print(kable(quast_tab) %>%
  #   kable_styling(bootstrap_options = c("striped", "hover","responsive")))
  
  DT::datatable(quast_tab, rownames= FALSE)
  
    if(dim(quast_tab)[1] != 0){
     wbq <- createWorkbook()
     addWorksheet(wbq, "quast")
  ...
  # library(reshape2)
  # library(ggplot2)
  # long_df <- melt(quast_tab, id.vars = c("sample"), variable.name = "Test", 
  #               value.name = "Value")
  # 
  # ggplot(data=long_df, aes(x=test, y=Value, fill=sample)) +
...
try({
  ...
  library(DT)
  DT::datatable(mlst_tab, rownames= FALSE)
...
```
### AMRfinfer+ virulence  {.tabset}

```{r amrfinder virulence, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
 amrfinder_db_vir <- list.files(path=paste0(path_output, "/amrfinder/"), full.names=TRUE, recursive=TRUE)

   amrfinder_db_function_virulence <- function(i){
     amr <- read.csv(i, sep= "\t")
     if(nrow(amr) > 0){
      amr <- amr[amr$Type == "VIRULENCE",]
      if(nrow(amr) > 0) {
   ...
   amr_tab_vir <- lapply(amrfinder_db_vir, amrfinder_db_function_virulence)
   amr_tab_vir <- amr_tab_vir[!sapply(amr_tab_vir,is.null)]
   amr_tab_vir <- amr_tab_vir[sapply(amr_tab_vir, nrow)>0]

  for (i in 1:length(amr_tab_vir)) {
      cat("####", as.data.frame(amr_tab_vir[i])$Sample[1], "<br>", "\n")
    print(kable(as.data.frame(amr_tab_vir[i]), row.names = FALSE) %>%
      kable_styling(bootstrap_options = c("striped", "hover","responsive"),
                    fixed_thead = T) %>%
         scroll_box(width = "100%", height = "330px"))
    cat("\n", "<br>", "\n\n")
   ...
   amr_tab1_vir <- as.data.frame(do.call(rbind, lapply(amrfinder_db_vir, amrfinder_db_function_virulence)))
   addWorksheet(wbv, "AMRfinder")
   writeData(wbv, sheet = "AMRfinder", amr_tab1_vir) 
...
```
### AMRfinder plus virulence
```{r AMRfinder vir plot, echo=FALSE, message=FALSE, warning=FALSE, results='asis', fig.align='center'}
 amrfinder <- list.files(path=paste0(path_output, "/amrfinder/"), full.names=TRUE, recursive=TRUE)
...
virulencefinder <- list.files(path=paste0(path_output, "/virulencefinder/"), pattern = "results_tab.tsv", full.names=TRUE, recursive=TRUE)
   ...
   virulencefinder_function <- function(i){
     ...
     if(nrow(vf) > 0){
      ...
      vf$Cv1 <- gsub("/.*","",vf$Coverage)
      vf$Cv1 <- trimws(vf$Cv1)
      vf$Cv1 <- as.numeric(vf$Cv1)
      vf$Cv2 <- gsub(".*/","",vf$Coverage)
      vf$Cv2 <- trimws(vf$Cv2)
      vf$Cv2 <- as.numeric(vf$Cv2)
      vf$Coverage <- (vf$Cv1/vf$Cv2)*100
      vf <- vf[vf$Identity >= identity, ]
...
```{r Plasmids, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
 abricate_plfd <- list.files(path=paste0(path_output, "/abricate/"), pattern = "plasmidfinder", full.names=TRUE, recursive=TRUE)
  ...
  writeData(wbp, sheet = "Plasmidfinder", pd1_tab)
...
## *Escherichia* {.tabset .tabset-fade .tabset-pills #eschr}
...
#Da sistemare, ec_tab deve essere una lista
   # amr_tab <- lapply(amrfinder_db, amrfinder_db_function)
   # amr_tab <- amr_tab[!sapply(amr_tab,is.null)]
   # amr_tab <- amr_tab[sapply(amr_tab, nrow)>0]
...
if(dim(ec_tab)[1] != 0){
  #   for (i in 1:length(ec_tab)) {
  # cat("####", as.data.frame(ec_tab[i])$Sample[1], "<br>", "\n")
  #   print(kable(as.data.frame(ec_tab[i]), row.names = FALSE) %>%
  #     kable_styling(bootstrap_options = c("striped", "hover","responsive"),
  #                   fixed_thead = T) %>%
  #        scroll_box(width = "100%", height = "330px"))
  #   cat("\n", "<br>", "\n\n")
...
```{r Kleborate-escherichia, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
 kleborate_esch <- list.files(path=paste0(path_output, "/kleborate_escherichia/"), pattern = "", full.names=TRUE, recursive=TRUE)
  ...
  klb_es_tab <- lapply(kleborate_esch, kleborate_esch_function)
  klb_es_tab <- klb_es_tab[!sapply(klb_es_tab,is.null)]
  klb_es_tab <- klb_es_tab[sapply(klb_es_tab, nrow)>0]

   if(length(klb_es_tab) > 0) {
    for (i in 1:length(klb_es_tab)) {
  cat("####", as.data.frame(klb_es_tab[i])$Sample[1], "<br>", "\n")
    print(kable(as.data.frame(klb_es_tab[i]), row.names = FALSE) %>%
      kable_styling(bootstrap_options = c("striped", "hover","responsive"),
                    fixed_thead = T) %>%
         scroll_box(width = "100%", height = "330px") %>% 
      footnote(general = "* imprecise allele matches")) 
    cat("\n", "<br>", "\n\n")
    }
...
ecoh <- list.files(path=paste0(path_output, "/abricate_ecoli/"), pattern = "ecoh", full.names=TRUE, recursive=TRUE)
  ...
  ec_tab <- lapply(ecoh, ecoh_function)
  ec_tab <- ec_tab[!sapply(ec_tab,is.null)]
  ec_tab <- ec_tab[sapply(ec_tab, nrow)>0]
...
```{r ecoli_vf, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
 ecoli_vf <- list.files(path=paste0(path_output, "/abricate_ecoli/"), pattern = "ecoli_vf", full.names=TRUE, recursive=TRUE)
  ...
  evf_tab <- lapply(ecoli_vf, ecoli_vf_function)
  evf_tab <- evf_tab[!sapply(evf_tab,is.null)]
  evf_tab <- evf_tab[sapply(evf_tab, nrow)>0]

  if(length(evf_tab) > 0) {
    for (i in 1:length(evf_tab)) {
    cat("####", as.data.frame(evf_tab[i])$Sample[1], "<br>", "\n")
    print(kable(as.data.frame(evf_tab[i]), row.names = FALSE) %>%
      kable_styling(bootstrap_options = c("striped", "hover","responsive"),
                    fixed_thead = T) %>%
         scroll_box(width = "100%", height = "330px"))
    cat("\n", "<br>", "\n\n")
...
fim <- list.files(path=paste0(path_output, "/fimtyper/"), pattern = "_tab", full.names=TRUE, recursive=TRUE)
  ...
  fim_tab <- lapply(fim, fim_function)
  fim_tab <- fim_tab[!sapply(fim_tab,is.null)]
  fim_tab <- fim_tab[sapply(fim_tab, nrow)>0]

  
  if(length(fim_tab) > 0) {
    for (i in 1:length(fim_tab)) {
      cat("####", as.data.frame(fim_tab[i])$Sample[1], "<br>", "\n")
      print(kable(as.data.frame(fim_tab[i]), row.names = FALSE) %>%
      kable_styling(bootstrap_options = c("striped", "hover","responsive"),
                    fixed_thead = T) %>%
         scroll_box(width = "100%", height = "330px"))
      cat("\n", "<br>", "\n\n")
    ...
    fim1_tab <- as.data.frame(do.call(rbind, lapply(fim, fim_function)))
    addWorksheet(wbec, "fimtyper")
    writeData(wbec, sheet = "fimtyper", fim1_tab)
  }
```

### ClermonTyping {.tabset}
...
clermontyping <- list.files(path=paste0(path_output, "/ClermonTyping/"), pattern = "_phylogroups", full.names=TRUE, recursive=TRUE)
   ...
   clermontyping_function <- function(i){
     ...
     if(nrow(cle) > 0 & (!is.na(cle$V2) | !is.na(cle$V3) | 
                         !is.na(cle$V4) | !is.na(cle$V5)))  {
  ...
  cle_tab <- lapply(clermontyping, clermontyping_function)
  cle_tab <- cle_tab[!sapply(cle_tab,is.null)]
  cle_tab <- cle_tab[sapply(cle_tab, nrow)>0]

  
    for (i in 1:length(cle_tab)) {
      cat("####", as.data.frame(cle_tab[i])$Sample[1], "<br>", "\n")
      print(kable(as.data.frame(cle_tab[i]), row.names = FALSE) %>%
      kable_styling(bootstrap_options = c("striped", "hover","responsive"),
                    fixed_thead = T) %>%
         scroll_box(width = "100%", height = "330px"))
      cat("\n", "<br>", "\n\n")
    ...
    if(length(cle_tab) > 0) {
      cl <- cle_tab[[1]]
      if(dim(cl)[1] != 0){
      cle1_tab <- as.data.frame(do.call(rbind, lapply(clermontyping, clermontyping_function)))
    
      addWorksheet(wbec, "ClermonTyping")
      writeData(wbec, sheet = "ClermonTyping", cle1_tab) 
  }
    }
```
...
```{r Shigatyper, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
 shigatyper <- list.files(path=paste0(path_output, "/shigatyper/"), pattern="_def.tsv", full.names=TRUE, recursive=TRUE)
  ...
  if(dim(shiga_tab)[1] != 0){
   ...
   writeData(wbshi, sheet = "Shigatyper", shiga1_tab)
    }
...
```{r ShigEiFinder, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
 ...
 shige <- list.files(path=paste0(path_output, "/ShigEiFinder/"), pattern = ".txt", full.names=TRUE, recursive=TRUE)
  ...
  shige_tab <- as.data.frame(do.call(rbind, lapply(shige,  shige_function)))

  print(kable(shige_tab) %>%
    kable_styling(bootstrap_options = c("striped", "hover","responsive")))
  
   if(dim(shige_tab)[1] != 0){
    shige1_tab <- as.data.frame(do.call(rbind, lapply(shige,  shige_function)))
    addWorksheet(wbshi, "ShigEiFinder")
    writeData(wbshi, sheet = "ShigEiFinder", shige1_tab) 
...
```{r meningotype, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
 ...
 meningotype <- list.files(path=paste0(path_output, "/meningotype/"), pattern = ".txt", full.names=TRUE, recursive=TRUE)
  ...
  men_tab <- as.data.frame(do.call(rbind, lapply(meningotype,  meningotype_function)))

  print(kable(men_tab) %>%
    kable_styling(bootstrap_options = c("striped", "hover","responsive")))
  
   if(dim(men_tab)[1] != 0){
  men1_tab <- as.data.frame(do.call(rbind, lapply(meningotype,  meningotype_function)))
   wbneis <- createWorkbook()
   addWorksheet(wbneis, "meningotype_N.meningitis")
   writeData(wbneis, sheet = "meningotype_N.meningitis", men1_tab)
  }
...
```{r ngmaster, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
 ...
 ngmaster <- list.files(path=paste0(path_output, "/ngmaster/"), pattern = ".txt", full.names=TRUE, recursive=TRUE)
  ...
  ng_tab <- as.data.frame(do.call(rbind, lapply(ngmaster,  ngmaster_function)))

  print(kable(ng_tab) %>%
    kable_styling(bootstrap_options = c("striped", "hover","responsive")))
  
   if(dim(ng_tab)[1] != 0){
  ng1_tab <- as.data.frame(do.call(rbind, lapply(ngmaster,  ngmaster_function)))
   addWorksheet(wbneis, "ngmaster_N.gonorrhoeae")
   writeData(wbneis, sheet = "ngmaster_N.gonorrhoeae", ng1_tab) 
...
pasty <- pasty[-c(a,b)]
  ...
  pa_tab <- as.data.frame(do.call(rbind, lapply(pasty,  pasty_function)))

  print(kable(pa_tab) %>%
    kable_styling(bootstrap_options = c("striped", "hover","responsive")))
  
   if(dim(pa_tab)[1] != 0){
  pa1_tab <- as.data.frame(do.call(rbind, lapply(pasty,  pasty_function)))
    write.xlsx(pa1_tab, file=paste0(path_excel, "/P.aeruginosa_table.xlsx"),   sheetName = "Pasty", col.names = TRUE, row.names = FALSE, append=TRUE)
  }
